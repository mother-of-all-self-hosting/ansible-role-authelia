...
#
# Tags look like `v<Authelia version>-<release>`:
#
# - if defaults/main.yml points at an Authelia version that has never been
#   released, the release counter restarts at 0 (`v4.37.5-0`)
# - otherwise the counter is incremented (`v4.37.5-1`), but only if something
...
# `authelia_version` is the variable that Renovate bumps (it carries the
# `# renovate:` annotation). The container image variables next to it are
# derived from it through Jinja and are not literal versions, so they must
# never be what this is keyed on.
version="$(sed -nE 's|^authelia_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Authelia version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `4.37.5`), but the tags
# do (`v4.37.5-0`). Stripping any `v` before prepending one keeps this correct
